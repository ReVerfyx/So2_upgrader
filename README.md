...
| --- | --- |
| Приложение | **Northflank** (не засыпает) или **Render** (проще, засыпает через 15 мин) |
| База данных | **Aiven** — 1 ГБ, без лимита времени работы, с бэкапами |
| Домен | поддомен платформы (`*.code.run`, `*.onrender.com`), `is-a.dev`, `eu.org`, DuckDNS |
| HTTPS | выпускает платформа |

В репозитории уже лежат `Dockerfile`, `render.yaml` (Render Blueprint) и
`koyeb.yaml` (для старых аккаунтов Koyeb). При первом запуске с `SEED_ON_START=true` сервис сам применит
